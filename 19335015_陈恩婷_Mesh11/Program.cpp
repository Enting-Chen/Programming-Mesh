...
int partition(int* cells, int* pecell, double* p_x)
{
	return 0;
}
...
int running(int argc, char* argv[])
{
	...
	int* g_becell = 0, * g_ecell = 0, * g_bound = 0, * g_bedge = 0, * g_edge = 0,
		* g_cell = 0;
	double* g_x = 0, * g_q = 0, * g_qold = 0, * g_adt = 0, * g_res = 0, *new_x = 0;

	//local mesh
	int nnode, ncell, nedge, nbedge;
	int* becell, * ecell, * bound, * bedge, * edge, * cell;
	double* x, * q, * qold, * adt, * res, *x_for_area;
	double rms, g_rms;
	...
	x_for_area = (double*)malloc(3 * 4 * ncell * sizeof(double));
	...
	scatter_double_array(new_x, x_for_area, comm_size, 4*g_ncell, 4*ncell, 3);
	...
	//gather_double_array(...);  //gather q to root process
